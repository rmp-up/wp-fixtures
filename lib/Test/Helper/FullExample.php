...
trait FullExample
{
    public function assertExampleContainsAllFields(string $expectedClass, string $exampleKey = '', int $exampleIndex = 0)
    {
        static::assertTrue(class_exists($expectedClass), 'Unknown class: ' . $expectedClass);

        $example = $this->fetchExampleAsArray($expectedClass, $exampleKey, $exampleIndex);
    ...
    protected function assertMatchesExample($exampleClass, $exampleName, $storedData)
    {
        $exampleAsArray = $this->fetchExampleAsArray($exampleClass, $exampleName, 0);

        static::assertEquals($storedData, $exampleAsArray);
    }
     ...
     * @return mixed
     */
    protected function fetchExampleAsArray(string $expectedClass, string $exampleKey, int $exampleIndex)
    {
